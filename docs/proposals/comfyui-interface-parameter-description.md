...
ComfyUI 工作流中的每个节点具有不同参数，参数含义和影响需要明确记录，使算法工程师能够维护工作流并生成稳定参数 Schema。后端只读取算法服务 API 声明的业务参数，不直接接触节点参数。

## 目标用户

- 配置 ComfyUI 工作流的算法工程师
- 构建内部 Parameter Capability 的算法服务开发者
- 核对算法 API 中业务参数来源的后端开发者
...
- 逐个节点描述工作流参数的名称、范围、选项和专业含义
- 每份参数清单绑定 ComfyUI 版本、工作流模板哈希、custom node 版本锁和模型版本
- 从实际工作流与节点定义导出参数清单，并在 CI 或发布校验中检测文档漂移
- 标记参数是后端可配置还是仅算法内部使用；只有后端可配置参数进入算法 API Schema
- 覆盖以下节点类型：
  - Load Image（四视图输入）
...
- 不规定参数的默认取值或推荐配置
- 不提供参数组合的最佳实践
- 不向后端、前端或 Agent 暴露 ComfyUI 节点 ID、graph、服务器路径和算法内部参数

## 关键决策与依据

本提案是算法组维护 Parameter Capability 的内部事实资料。节点与范围由可复现导出结果确定，人工文档只补充业务语义；算法服务 API 是后端可见参数的唯一契约。
...
- 现状：节点或工作流升级后，手写参数范围可能继续存在但已经失真。
- 提议后的行为：参数文档绑定依赖版本和模板哈希，并与导出清单自动比对。
- 验收：节点新增、删除或修改参数时，发布校验失败并列出差异；更新文档和版本锁后才能发布。

### 例子 5：算法黑盒边界

- 现状：后端可能直接依赖节点参数名，导致算法内部升级传播到业务层。
- 提议后的行为：后端只看到算法 API 声明的业务参数，节点参数映射留在算法服务内部。
- 验收：节点名或内部参数变化但业务语义不变时，后端 API 请求无需修改。
