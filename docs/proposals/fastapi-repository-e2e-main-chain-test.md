...
- 现状：没有 API 或全链路测试。
- 提议后的行为：通过 API 创建 Project、提交 STL 输入；RuleBasedAgent 路由经过上传、检查、切片、确认和 mock 打印，最终到达 `picked_up`。
- 验收：端到端测试覆盖完整路径，所有 TaskLog 记录共享同一 `traceId`；最终状态为 `picked_up`。

### 例子 2：检查未通过阻断

- 现状：无。
- 提议后的行为：提交一个检查失败的模型输入，主链在检查阶段被阻断，状态不会进入切片或打印。
- 验收：API 查询显示当前状态停留在检查失败阶段，TaskLog 记录阻断原因。
